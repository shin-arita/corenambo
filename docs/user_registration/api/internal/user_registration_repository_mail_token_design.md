...
| 条件 | 呼び出し | status | retry_count | next_attempt_at |
|------|---------|--------|-------------|-----------------|
| SMTP送信失敗（接続エラー等） | MarkRetry | pending | +1 | 現在 + 5分 |
| リトライ上限到達（WorkerMaxRetryCount 以上） | MarkFailed | failed | 変更なし | 現在 + 24時間 |
| payload 不正（JSON parse 失敗） | MarkFailed | failed | 変更なし | 現在 + 5分 |
| 送信前に確定できる失敗（NonRetryableMailError） | MarkFailed | failed | 変更なし | 現在 + 24時間 |
...
- `MarkFailed` は `status = 'failed'` に設定し、`retry_count` は増やさない（最終状態なので変更不要）。`next_attempt_at` は常に更新する
- `status = 'failed'` のレコードは `FetchPending` の取得対象にならないため、`next_attempt_at` は通常の送信フローでは参照されない。調査・手動復旧・将来的な再投入設計のために記録として残る
- invalid payload（JSON parse 失敗）はリトライしても解消しないため、即 MarkFailed（non-retryable failure）

### NonRetryableMailError

`mail.NonRetryableMailError` は送信前に確定できる non-retryable なエラーを表す型。
worker は `errors.As` でこの型を検出し、`MarkRetry` の代わりに `MarkFailed` を呼ぶ。

non-retryable とみなす条件（`mail` パッケージ内で返す）：
...
`mail_outboxes.payload` は Go 側で `json.Marshal` した結果を `string` として渡し、DB 側では `jsonb NOT NULL` カラムに保存する。
...
- 1秒間隔でポーリング
- pending を取得
- リトライ制御あり（NonRetryableMailError / retryable error / リトライ上限 の3分岐）
